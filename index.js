...
app.use(
  cors({
    origin: [
      "http://localhost:5173",
      "https://job-portal-a519b.web.app",
      "https://job-portal-a519b.firebaseapp.com", //remove the slash form the last
    ],
    credentials: true,
  })
);
...
const verifyToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decode) => {
    if (err) {
      return res.status(401).send({ message: "Unauthorizend access" });
    }
    req.user = decode;
    next();
  });
...
async function run() {
  try {
    ...
    app.get("/job-application", verifyToken, async (req, res) => {
      ...
      //   token user email !== qurery email (current email)
